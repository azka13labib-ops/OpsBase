...
create policy "Authenticated users can read warnings" on warnings for
select using (
        auth.role () = 'authenticated'
    );

create policy "Authenticated users can read mod_actions" on mod_actions for
select using (
        auth.role () = 'authenticated'
    );

create policy "Authenticated users can read events" on events for
select using (
        auth.role () = 'authenticated'
    );

create policy "Authenticated users can read event_rsvps" on event_rsvps for
select using (
        auth.role () = 'authenticated'
    );
