...
3. Jalankan smoke test require semua module utama (`api/server.js`,
   `supabase/client.js`, `services/push.js`, semua file di `commands/moderation/`
   dan `events/`) untuk mendeteksi module yang salah nama/path.
...
## Tahap 4 — Deploy slash command & jalankan bot
...
## Tahap 6 — Setup Firebase di Flutter (push notification)
...
## Tahap 7 — Setup deep link OAuth Discord
